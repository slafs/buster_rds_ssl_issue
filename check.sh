...
# Usage:
#   ./check.sh postgres://user:pass@host:port/dbname
DSN=${1:?"DSN needed"}
...
loginfo "checking psycopg2 connection on stretch..."
docker run -it --rm -e SECURE_DSN --entrypoint python ${STRETCH_IMAGE} \
    -c "${PYTHON_COMMAND}"

loginfo "checking psycopg2 connection on buster..."
...
loginfo "checking psql on buster..."
